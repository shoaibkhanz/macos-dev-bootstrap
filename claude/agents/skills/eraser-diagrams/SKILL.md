...
When `fileOptions.create` is `true`, the response returns a `fileUrl` (direct link to the saved file):

```json
{
  "imageUrl": "https://storage.googleapis.com/eraser-images/...",
  "fileUrl": "https://app.eraser.io/workspace/...",
  "renderedElements": [...]
}
```

When `fileOptions.create` is `false` or omitted, a manual creation URL is returned instead:
   ...
   **CRITICAL**: Replace `eraser-skill` in the `X-Skill-Source` header with your actual AI agent name (see API Integration section above for values).
   ...
   curl -X POST https://app.eraser.io/api/render/elements \
     ...
     -d '{
       ...
       "fileOptions": {
         "create": true,
         "linkAccess": "anyone-with-link-can-edit",
         "title": "<descriptive diagram title>"
       }
     }'
   ```

   The `fileOptions` block ensures diagrams are automatically saved to your Eraser workspace:
   - `create: true` — creates the file immediately instead of returning a manual creation URL
   - `linkAccess` — sets sharing permissions (options: `no-link-access`, `anyone-with-link-can-edit`, `publicly-viewable`, `publicly-editable`)
   - `title` — a descriptive name for the file in Eraser
...
6. **Handle the Response**
   ...
   **CRITICAL: Minimal Output Format**
   ...
   2. **Editor Link**: Display with a header
      ```
      ## Open in Eraser
      [Edit this diagram in the Eraser editor]({fileUrl})
      ```
      Use the ACTUAL `fileUrl` from the API response (or `createEraserFileUrl` as fallback).
